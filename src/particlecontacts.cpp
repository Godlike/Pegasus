...
void pegas::ParticleContact::resolveVelocity(pegas::real const duration) {
  ...
  real const accCausedSepVelocity =
      accCausedVelocity * mContactNormal * duration;
  ...
  if (mB) {
    mB->setVelocity(mB->getVelocity() +
                    impulsePerIMass * -mB->getInverseMass());
  }
}
...
void pegas::ParticleContact::resolveInterpenetration(
  ...
  Vector3 const movePerIMass =
      mContactNormal * (-mPenetration / totalInverseMass);
  ...
  while (mIterationsUsed < mIterations) {
    ...
    auto max_it = std::max_element(
        ...
        [](ParticleContact::Ptr const &a, ParticleContact::Ptr const &b) {
          return a->calculateSeparatingVelocity() <
                 b->calculateSeparatingVelocity();
        });
