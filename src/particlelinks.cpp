...
unsigned int pegas::ParticleCabel::fillContact(
        pegas::ParticleContact::Ptr & contact, unsigned int const limit
    ) const
{
    if (!contact)
    {
    ...
    contact = std::make_shared<ParticleContact>(
        a, b, restitution, normal, length - maxLength
    );

    return 1;
}

unsigned int pegas::ParticleRod::fillContact(
        pegas::ParticleContact::Ptr & contact, unsigned int const limit
    ) const
{
    if (!contact)
